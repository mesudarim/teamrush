<!--
  MultiSelect — like MultipleChoice but with multiple correct answers.
  Player must find ALL correct choices. Each click is irreversible:
    correct → green  | wrong → red
  Emits 'correct' once all correct choices are selected; 'wrong' on each bad click.
-->
<script setup>
...
const correctSelected = ref(new Set())
const wrongIndices    = ref(new Set())

const choices = computed(() => props.question.choices ?? [])
const totalCorrect = computed(() => choices.value.filter(c => c.isCorrect).length)
const solved = computed(() => totalCorrect.value > 0 && correctSelected.value.size >= totalCorrect.value)
...
const stateOf = (idx) => {
  if (correctSelected.value.has(idx)) return 'correct'
...
const handleChoice = (idx) => {
  ...
  if (correctSelected.value.has(idx) || wrongIndices.value.has(idx)) return

  if (choices.value[idx]?.isCorrect) {
    correctSelected.value = new Set([...correctSelected.value, idx])
    playCorrect()
    if (correctSelected.value.size >= totalCorrect.value) {
      emit('correct')
    }
  } else {
  ...
  <BaseMission :checkpoint="checkpoint" :config="config" :show-title="false">
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-center gap-2">
        <div
          v-for="i in totalCorrect"
          :key="i"
          :class="['w-3 h-3 rounded-full border-2 transition-all',
            i <= correctSelected.size
              ? 'bg-green-500 border-green-500 scale-110'
              : 'bg-transparent border-slate-500']"
        />
        <span class="text-xs text-slate-400 ms-1">
          {{ t('missions.multiSelect.progress', { found: correctSelected.size, total: totalCorrect }) }}
        </span>
      </div>
      ...
      <div class="space-y-2">
        <button
          ...
          :disabled="solved || correctSelected.has(idx) || wrongIndices.has(idx)"
          :class="[
        ...
        >
          <div class="flex items-center gap-3">
            <div :class="[
              'w-7 h-7 rounded-lg border-2 flex items-center justify-center text-xs font-bold shrink-0 transition-all',
              stateOf(idx) === 'correct' ? 'border-green-500 bg-green-500 text-white' :
              stateOf(idx) === 'wrong'   ? 'border-red-600 text-red-400' :
              'border-slate-500 text-slate-400'
            ]">
              {{ stateOf(idx) === 'correct' ? '✓' : stateOf(idx) === 'wrong' ? '✕' : '' }}
            </div>
