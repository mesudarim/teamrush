...
const checkAnswer = (input) => {
  const heAnswer = (props.question.answer   ?? '').trim()
  const enAnswer = (props.question.answerEn ?? '').trim()
  const expected = (locale.value === 'en' && enAnswer) ? enAnswer : heAnswer

  // Numeric tolerance: ±15° for degree answers (language-agnostic)
